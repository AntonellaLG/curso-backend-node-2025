//utilizo * para importar una función de models que se llama igual a una que tengo acá en el controlador, sino estaría redeclarando y da error
...
export const getAllProducts = (req, res) => { //uso la función getAllProducts con su módulo para evitar el conflicto
    res.json(model.getAllProducts()); //necesito los products, los importo
};
...
export const searchProduct = (req, res) => { 
    ...
    const products = model.getAllProducts(); //pido los productos a model
...
export const createProduct = (req, res) => { //NECESITO USAR UN MIDDLEWARE, EL DE LA LINEA 11 del index.js
    ...
    //res.send('POST');
    const products = model.getAllProducts(); //pido los productos al servicio

    const { name, price } = req.body;

    const newProduct = model.createProduct({ name, price }); //creo un objeto con los datos que necesito pasar (lo que persistió en la bdd)
...
export const deleteProductById = (req, res) => {
    ...
    //TODO LO QUE SEA DATOS LO MANEJA EL MODELO
    const product = model.deleteProductById(productId);

    //EL CONTROLADOR RESPONDE LA CLIENTE
